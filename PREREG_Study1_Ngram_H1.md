...
| 2026-08-12 | Snippet SOURCE substituted: Google Books API → Internet Archive full-text search + Search Inside. The anonymous Books API daily quota was continuously exhausted 2026-08-10 22:44 CEST → 2026-08-12 (shared global consumer bucket; 429 on every probe over 43+ hours including a 30-hour automated wait; legacy GData endpoint dead; HathiTrust Cloudflare-blocked). New method per term and era (still era-split, still first 25 unique, still skipping <15-char fragments): `advancedsearch.php` query `"{term}" AND year:[1500 TO 1950|1950 TO 2026] AND format:("Abbyy GZ") AND mediatype:texts` → per item in search order, metadata (server/dir) → `inside.php` (Search Inside) → OCR lines containing the term as snippets. | Data-source availability (no key, no account, automated; only viable free full-text book source). | Era balance preserved; snippet text is OCR'd line-level content — OCR artifacts may raise AMBIG rates (reported); post-1950 IA holdings skew toward public-domain reprints and long-tail titles vs Google Books mainstream (disclosed in the paper like the OSF-deviation note); precision/flag rule and all coding rules unchanged. |
