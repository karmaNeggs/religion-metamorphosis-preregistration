...
| 2026-08-11 | H2e mental-health predictor operationalized from BRFSS state-level data instead of CDC PLACES: PLACES publishes no state-level release (county/place/tract/ZCTA only), so the pre-registered fallback "BRFSS if PLACES is unavailable" applies. Source: CDC Socrata dataset `5eh7-pjx8` (BRFSS – Mental Health Indicators), question = "mental health not good" (frequent mental distress, ≥14 days, past 30), demographics = Total; per-state mean over available years 2022–2024 (2020–2021 not in this dataset — prereg window was 2020–2023; actual window recorded here). | Data availability. | Predictor column `mental` in `data/raw/trends/states/cdc_places_mental.csv` (index = state name, n = 51); the paper notes the actual source/window per protocol's "note in paper if version differs". |
| 2026-08-11 | Trends collection day 2: remaining reps collected (A now 4/5 reps, B1/B2/C/CHB1/CHB2 5/5; A_rep4 lost to a final 429), medians recomputed and written by the script; state-level region pulls (--states) run on the same day. | Google Trends rate limits. | Median-of-N with N per group reported; supersedes the interim medians in the 2026-08-10 entry (which remains as the record). |
